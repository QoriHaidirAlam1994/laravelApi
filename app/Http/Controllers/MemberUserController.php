...
class MemberUserController extends Controller
{
    ...
    {
        $this->validate($request, MemberUser::rules());
        ...
        $model = MemberUser::create($attributes);
    ...
    {
        $this->validate($request, MemberUser::accessTokenRules());
    ...
    public function refresh(Request $request)
    {
        ...
        if (!$access_token = $this->refreshAccesstoken($headers['authorization'])) {
    ...
    {
        $this->validate($request, MemberUser::authorizeRules());


        if ($model = MemberUser::authorize($request->all())) {
            ...
            $response = [
                'status' => 0,
                'errors' => [
                               "password"=>["Member_id or Password is wrong"]
                            ]
            ];
    ...
    public function update(Request $request, $id)
    {
        ...
        $model = $this->findModel($id,$request);
        $this->validate($request, MemberUser::updateRules($id));
    ...
    {
        $response = MemberUser::search($request);
        return response()->json($response, 200, [], JSON_PRETTY_PRINT)
            ->setCallback($request->input('callback'));
